...
# output 디렉토리를 스캔해서 새로 완료된 인덱싱 단계 목록을 반환
# start_time: 인덱싱 시작 시각 — 이 시각 이후 생성된 파일만 현재 실행 산출물로 인정
# reported_progress: 이미 보고한 최고 진행도 — 이보다 높은 단계만 반환
# 반환값: [(progress, message), ...] 순서 보장 (낮은 % → 높은 %)
